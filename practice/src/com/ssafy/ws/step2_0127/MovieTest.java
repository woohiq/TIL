...
public class MovieTest {
	public static void main(String[] args) {
		...
		List<Movie> SearchTest1 = MvManager.searchByTitle("HomeAlone"); // 이미 존재하는 영화 : 시리즈 아닌 영화 (제목으로 검색)
		for (int i = 0;SearchTest1!=null && i<SearchTest1.size(); i++) {
			System.out.println(SearchTest1.get(i).toString());
		}
		System.out.println("-------------");
		System.out.println("제목으로 영화 검색 : 해리포터");
		List<Movie> SearchTest2 = MvManager.searchByTitle("해리포터"); // 이미 존재하는 영화 : 시리즈 영화 (제목으로 검색)
		for (int i = 0; SearchTest2!=null && i<SearchTest2.size(); i++) { // 시리즈 영화의 경우 에피소드가 있으므로 에피소드도 출력
